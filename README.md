...
│   ├── staff-01〜05.jpg    … スタッフ写真 1000×1000（2026-09-08 支給。白背景の切り抜き PNG 736×648 を、5人同じ倍率 1.7 で 1000×1000 の白地に中央・下揃えで配置。胸の切れ目が枠の下端に来る）
│   │                         上から staff-01 葉月 / 02 良 / 03 由香里 / 04 典子 / 05 稔（掲載順は発注者指定）
│   └── （history-02 / 06 / 08 / 09 は未支給。枠は PHOTO WANTED のまま）
...
| 33 | STAFF の写真枠をスマホで 3:2 → 1:1 に | 支給写真（白背景のバストアップ）を 3:2 の横長で object-fit:cover すると頭が切れる。正方形なら PC（4:5、左右を1割ずつ切る）と同じ見え方になる（2026-09-08） |
| 34 | STAFF の偶数行は列幅も入れ替える（`.staff-row:nth-child(even){grid-template-columns:58% 42%}`） | プロトタイプは `order:2` だけで写真を右に送っていたため、写真が右の行は 58% 幅・左の行は 42% 幅と、行ごとに写真の大きさが変わっていた。実写真を入れて判明 |
...
3. スタッフ5名の名前・自由文（写真は 2026-09-08 に支給・掲載済み。名前は alt にのみ入れ、表示名・本文は「スタッフ名」「※ここに自由文…」のまま）
